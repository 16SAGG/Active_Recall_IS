...
func _draw() -> void:
	...
	for _i_v in item_value:
		...
		draw_circle_arc_poly(self.rect_size / 2, _max_radius, _percentage_traveled, _percentage_traveled + _item_percent, _item_color)
		
		_percentage_traveled += _item_percent
	
	var _small_circle_radius : float = _max_radius * 0.65
	draw_circle_arc_poly(self.rect_size / 2, _small_circle_radius, 0, 360, THEMES.DICT_THEMES[_current_theme]['SECONDARY'])

func draw_circle_arc_poly(var _center : Vector2, var _radius : float, var _angle_from : float, var _angle_to : float, var color : String) -> void:
	var _nb_points = 32
	var _points_arc = PoolVector2Array()
	_points_arc.push_back(_center)
	var _colors = PoolColorArray([color])

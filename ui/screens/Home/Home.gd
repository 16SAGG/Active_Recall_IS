...
func _load_data() -> Array:
	...
	for _d in USERDATA.decks_data:
		...
		_result_array.append(_create_deck_box(_d.title, _new_cards, _due_cards))
		
		var _pending_cards : int = _new_cards + _due_cards
		if _pending_cards > 0:
			_result_array.append(_create_pending_deck(_d.title, _pending_cards))
...
func _create_pending_deck(_title : String, _pending_cards : int) -> Control:

...
func _set_decks_data() -> void:
	var _d_q : String = "SELECT Deck.deck_id, Deck.title FROM Deck LEFT JOIN User ON Deck.user_id = User.user_id WHERE User.user_id = " + str(current_user)
...
func set_current_deck_data(_deck_id : int) -> void:
	var _d_q : String = "SELECT Deck.deck_id, Deck.title, Deck.days_on_streak, Deck.days_on_streak_record FROM Deck LEFT JOIN User ON Deck.user_id = User.user_id WHERE User.user_id = " + str(current_user) + " AND Deck.deck_id = " + str(_deck_id)
	...
	var _ds_q : String = "SELECT Deck_Set.new_cards_per_day, Deck_Set.state FROM Deck_Settings AS Deck_Set WHERE Deck_Set.deck_id = " + str(_data["deck_id"])
